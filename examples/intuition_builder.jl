...
##
# Matrix Condensation
import MarkovChainHammer.TransitionMatrix: perron_frobenius
P = perron_frobenius(markov_chain)
# Uncondensed matrix
U = zeros(length(markov_chain), length(markov_chain))
...
U[end, end] = 1
# Condenser
C = zeros(3, length(markov_chain))
for i in eachindex(markov_chain)
    C[markov_chain[i], i] = 1
end
C⁺ = pinv(C)
CU = C * U * C⁺

oC = zeros(length(markov_chain), 3)
for i in eachindex(markov_chain)
    oC[i, markov_chain[i]] = 1
end
oC⁺ = pinv(oC)
oCU = oC⁺ * U * oC
...
# Condenser
C = zeros(length(union(markov_chain)), length(markov_chain))
for i in eachindex(markov_chain)
    C[markov_chain[i], i] = 1
end
C⁺ = pinv(C)
...
## 
markov_chain = [collect(1:3)..., 1]
equivalence_markov_chain = [1 2 2 1]

C = zeros(length(union(equivalence_markov_chain)), length(union(markov_chain)))
for i in eachindex(union(markov_chain))
    C[markov_chain_equivalence[i], i] = 1
end
Cmod = copy(C)
Cmod[1, :] *= 0.0
Cmod[1, 1] = sum(C[1, :])
...
P = perron_frobenius(markov_chain)
equivalence_P = perron_frobenius(markov_chain_equivalence)
condensed_P = C * P * pinv(C)