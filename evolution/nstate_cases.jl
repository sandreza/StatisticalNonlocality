...
function nstate_ou(N, M; γ=1.0, ϵ=√2, arraytype=Array)
    ## Grid
    Ω = S¹(2π) × S¹(2)
    grid = FourierGrid(N, Ω, arraytype=arraytype)
    nodes, wavenumbers = grid.nodes, grid.wavenumbers
    ...
    ## Transition Matrix
    Δx = 2 / √(M-1)
    uₘ = 1 / sqrt(γ * 2 / ϵ^2) * [Δx * (i - (M-1) / 2) for i in 0:M-1]
    Q = ou_transition_matrix(M-1) * γ
    p = steady_state(Q)
    ...
    ## Allocate Fields with specific values
    [us[i] .= uₘ[i] * cos.(kˣ[2] * x .+ φs[i]) .* cos.(kʸ[2] .* y) for i in 1:M]
    [vs[i] .= uₘ[i] * sin.(kˣ[2] * x .+ φs[i]) .* sin.(kʸ[2] .* y) for i in 1:M]
